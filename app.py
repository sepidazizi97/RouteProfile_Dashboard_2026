...
import altair as altimport pandas as pdimport streamlit as st
...
st.set_page_config(page_title="BFT Spring Route Profiles",page_icon="🚌",layout="wide",)
...
st.markdown("""<style>.main-title {font-size: 38px;font-weight: 800;color: #12355B;margin-bottom: 2px;}
...
st.markdown('<div class="main-title">BFT Spring 2026 Route Profiles</div>',unsafe_allow_html=True,)
...
DATA_FOLDER = Path(file).parent / "data"

DIRECTION_COLORS = {"E": "#2563EB","W": "#F2B705","N": "#2563EB","S": "#F2B705","Inbound": "#F2B705","Outbound": "#2563EB","CW": "#2563EB","CCW": "#F2B705","Unknown": "#6B7280",}

OTP_COLORS = {"On-Time": "#2E86AB","Early": "#F6C85F","Late": "#D1495B",}
...
def standardize_column_names(df: pd.DataFrame,) -> pd.DataFrame:"""Convert source column names to lowercase snake_case."""
...
def safe_numeric(series: pd.Series,) -> pd.Series:"""Convert percentages and comma-formatted text to numeric."""
...
def normalize_route_name(value,) -> str | None:"""Keep route names such as 1, 26S, and 123S formatted correctly."""
...
def route_sort_key(value,):"""Sort routes naturally: 1, 2, 3, 10, 26, 26S, 123, 123S."""
...
def extract_route_from_filename(filename: str,) -> str:"""Extract the route from filenames such as:
...
def extract_trip_code(series: pd.Series,) -> pd.Series:"""Extract the middle portion of the Trip field.
...
def format_trip_time(series: pd.Series,) -> pd.Series:"""Extract and standardize trip times as HH."""
...
def extract_direction(series: pd.Series,) -> pd.Series:"""Extract direction from formats such as:
...
def clean_route_file(df: pd.DataFrame,route_name: str,) -> pd.DataFrame:"""Prepare one route-profile CSV for the dashboard."""
...
@st.cache_data(ttl=3600)def load_all_route_files() -> pd.DataFrame:"""Read and combine every CSV in the repository data folder."""
...
def temporal_bar_chart(data: pd.DataFrame,value_column: str,y_title: str,tooltip_title: str,height: int = 330,):"""Create a separate-direction chart using actual trip time."""
...
def combined_direction_bar_chart(data: pd.DataFrame,value_column: str,y_title: str,tooltip_title: str,aggregation: str = "sum",height: int = 360,):"""Create an evenly spaced grouped chart for both directions.
...
def otp_chart_for_direction(direction_df: pd.DataFrame,):"""Create a categorical OTP chart for one direction."""
...
def display_route_profile(route_df: pd.DataFrame,route_name: str,):"""Display all dashboard sections for one route."""
...
if spring_df.empty:st.error("No route CSV files were found. Add your files ""to the data folder in the GitHub repository.")

st.code(
    """

data/├── RouteProfile_Spring_Rt 1.csv├── RouteProfile_Spring_Rt 2.csv├── RouteProfile_Spring_Rt 3.csv└── ...""".strip())

st.stop()

route_options = sorted(spring_df["route_short_name"].dropna().unique().tolist(),key=route_sort_key,)

st.caption(f"{len(route_options)} route files loaded ""from the repository.")

route_tabs = st.tabs([f"Route {route}"for route in route_options])

for route_tab, route_name in zip(route_tabs,route_options,):with route_tab:route_df = spring_df[spring_df["route_short_name"]== route_name].copy()
